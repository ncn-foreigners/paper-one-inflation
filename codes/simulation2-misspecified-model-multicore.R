...
clusterExport(cl, c("N", "sims", "fn", "fn1", "fn2", "fn3", "fn4", "fn5"))
registerDoParallel(cl)

A <- foreach(k=1:sims, .combine = rbind, .packages = c("singleRcapture")) %dopar% {
  # covariates
  x1 <-  runif(n = N)
...
cl <- makeCluster(cores)
clusterExport(cl, c("N", "sims", "fn", "fn1", "fn2", "fn3", "fn4", "fn5", "fn6"))
registerDoParallel(cl)
...
A <- foreach(k=1:sims, .combine = rbind, .packages = c("singleRcapture")) %dopar% {
  ...
  rbind(c(
    sapply(list(M, M1, M2, M3, M4, M5, M6), FUN = function(x) {if (is.null(x)) NA else popSizeEst(x)$pointEstimate}),
    sapply(list(M, M1, M2, M3, M4, M5, M6), FUN = function(x) {if (is.null(x)) NA else popSizeEst(x)$confidenceInterval[1,1]}),
    sapply(list(M, M1, M2, M3, M4, M5, M6), FUN = function(x) {if (is.null(x)) NA else popSizeEst(x)$confidenceInterval[1,2]}),
    sapply(list(M, M1, M2, M3, M4, M5, M6), FUN = function(x) {if (is.null(x)) NA else popSizeEst(x)$confidenceInterval[2,1]}),
    sapply(list(M, M1, M2, M3, M4, M5, M6), FUN = function(x) {if (is.null(x)) NA else popSizeEst(x)$confidenceInterval[2,2]})
  ))
}
...
resNormL    <- A[,(1:7) + 7]
resNormU    <- A[,(1:7) + 14]
resLogNormL <- A[,(1:7) + 21]
resLogNormU <- A[,(1:7) + 28]
...
A <- foreach(k=1:sims, .combine = rbind, .packages = c("singleRcapture")) %dopar% {
  ...
  x4 <- rbinom(n = N, prob = .5, size = 1)
  eta <- cbind(
    cbind(1, x1, x2, x3, x4) %*% c(-.5, -.23, .06, 0.25, 0.215),
    log(.6),
